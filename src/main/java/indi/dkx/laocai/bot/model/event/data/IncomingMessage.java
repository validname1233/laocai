...
public class IncomingMessage {
    ...
    public String[] getImageUrls() {
        ...
        .map(Segment::getData)
        .filter(data -> data instanceof IncomingImageSegmentData)
        .map(data -> ((IncomingImageSegmentData) data).tempUrl())
        .toArray(String[]::new);
    }
    ...
    public long[] getMentionedUserIds() {
        return segments.stream()
        .filter(segment -> "mention".equals(segment.getType()))
        .map(Segment::getData)
        .filter(data -> data instanceof MentionSegmentData)
        .mapToLong(data -> ((MentionSegmentData) data).user_id())
        .toArray();
    }
}
