...
class MessageText(BaseModel):
    type: MessageDataType = Field(description="消息类型")
...
class FileBaseData(BaseModel):
    ...
    thumb: Optional[str] = Field(description="缩略图路径")
    name: Optional[str] = Field(description="文件名")
    file: str = Field(description="文件ID")
    url: Optional[str] = Field(description="文件URL")
...
class ImageData(FileBaseData):
    summary: Optional[str] = Field(description="图片摘要")
    sub_type: Optional[PicSubType] = Field(description="图片子类型")
...
class MessageJson(BaseModel):
    type: MessageDataType = Field(description="消息类型")
    data: Any = Field(description="JSON数据")

class JsonData(BaseModel):
    config: Optional[JsonConfig] = Field(description="配置")
...
class MessageDice(BaseModel):
    type: MessageDataType = Field(description="消息类型")
    data: DiceData = Field(description="骰子数据")
...
class MessageRPS(BaseModel):
    type: MessageDataType = Field(description="消息类型")
    data: RPSData = Field(description="RPS数据")
